...
- **Score conversion:** Crafty's `pv->pathv` is already **side-to-move-relative centipawns** (`PAWN_VALUE == 100`, positive = good for the side to move) — exactly what UCI wants, so `cp = pv->pathv` directly, with NO `wtm` negation. (NOTE: an earlier draft of this plan wrongly stated `pv->pathv` is White-relative and used `cp = wtm ? pv->pathv : -pv->pathv`; that was corrected during implementation after empirical verification. The Task 1/Task 2 code snippets below still show the original `wtm ?`/`-pv->pathv` form — disregard the negation; the shipped `UCIInfo` assigns `score = pv->pathv` directly.) Mate: `MateScore(s)` is `Abs(s) > 32000`; `MATE == 32768`; mate distance in full moves = `(MATE - Abs(pv->pathv) + 1) / 2`; emit `score mate N` when the side to move is mating (`pv->pathv > 0`), `score mate -N` when being mated.
